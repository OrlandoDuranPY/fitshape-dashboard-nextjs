...
export const trainingPlanSchema = z.object({
  user_uuid: z.string({error: "El usuario es requerido"}).min(1, "El usuario es requerido"),
  coach_uuid: z.string({error: "El entrenador es requerido"}).min(1, "El entrenador es requerido").optional().nullable(),
  name: z.string({error: "El nombre es requerido"}).min(1, "El nombre es requerido").max(255, "Máximo 255 caracteres"),
  ...
  starts_at: z
    ...
    .refine((val) => val > localToday(), "La fecha de inicio debe ser posterior a hoy"),
  ends_at: z
